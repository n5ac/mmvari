...
void __fastcall TLogListDlg::UpdateLogCount(int sw)
{
	...
	if( sw ){
		...
		char bf[256];
		if( Log.IsOpen() ){
			sprintf(bf, sys.m_MsgEng ? "%s - [%u QSO data(s)]" : "%s - [%u件のデータがあります]", Log.m_FileName.c_str(), Log.GetCount());
        }
        else {
...
{
	if( YesNoMB( sys.m_MsgEng ? "Delete Selection.(will not be able to restore)  Are you sure?":"現在選択されている範囲を削除します.削除した内容は復元できません\r\n\r\nよろしおまっか？") == IDYES ){
...
{
	if( YesNoMB( sys.m_MsgEng ? "It may take substantial time. Are you sure?":"この処理はメチャンコ時間がかかるかも知れません.\r\n\r\nよろしおまっか？" ) == IDYES ){
		int top = Grid->Selection.Top - 1;
		int end = Grid->Selection.Bottom - 1;
		if( top == end ){
			top = 0;
...
void __fastcall TLogListDlg::KMTextReadClick(TObject *Sender)
{
	...
	OpenDialog->InitialDir = sys.m_LogDir;
	if( OpenDialog->Execute() == TRUE ){
...
void __fastcall TLogListDlg::KMTextWriteClick(TObject *Sender)
{
	...
	SaveDialog->InitialDir = sys.m_LogDir;
	...
	switch(r){
		...
		case IDNO:
			if(  sys.m_MsgEng ){
				...
				r = OkCancelMB("現在MMVARIが保持しているログデータを全て失います.\r\n\r\nほんまにええでっか？");
			}
			switch(r){
				case IDOK:
					Log.DeleteAll();
...
void __fastcall TLogListDlg::LoadMmlogText(LPCSTR pName)
{
	...
	if( fp != NULL ){
		...
		ErrorMB(sys.m_MsgEng ? "Can't open '%s'" : "'%s'が見つかりません.", pName);
	}
	UpdateLogCount(1);
}
//---------------------------------------------------------------------------
void __fastcall TLogListDlg::SaveMmlogText(LPCSTR pName)
{
	...
	if( fp != NULL ){
		...
		if( fclose(fp) ){
			ErrorMB( sys.m_MsgEng ? "Can't save to '%s'" : "'%s'が正しく作成できませんでした.", pName);
		}
	}
	else {
		ErrorMB(sys.m_MsgEng ? "Can't save to '%s'" : "'%s'が作成できません.", pName);
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TLogListDlg::KExportLog200Click(TObject *Sender)
{
	...
	SaveDialog->InitialDir = sys.m_ExtLogDir;
	if( SaveDialog->Execute() == TRUE ){
		SaveLog200(AnsiString(SaveDialog->FileName).c_str());	//JA7UDE 0428
		SetDirName(sys.m_ExtLogDir, AnsiString(SaveDialog->FileName).c_str());	//JA7UDE 0428
	}
}
//---------------------------------------------------------------------------
void __fastcall TLogListDlg::KImportLog200Click(TObject *Sender)
{
	...
	OpenDialog->InitialDir = sys.m_ExtLogDir;
	if( OpenDialog->Execute() == TRUE ){
		LoadLog200(AnsiString(OpenDialog->FileName).c_str());	//JA7UDE 0428
		SetDirName(sys.m_ExtLogDir, AnsiString(OpenDialog->FileName).c_str());	//JA7UDE 0428
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TLogListDlg::KImportHamlogClick(TObject *Sender)
{
	...
	if( OpenDialog->Execute() == TRUE ){
		...
		SetDirName(sys.m_ExtLogDir, pName);
	}
}
//---------------------------------------------------------------------------
