...
void __fastcall CIndex::AllocIndex(int n)
{
	if( n >= m_IndexMax ){
...
void __fastcall CIndex::ClearIndex(void)
{
	m_IndexCnt = 0;
}

void __fastcall CIndex::WriteIndex(LPCSTR pCall, int n)
{
	AllocIndex(n);
...
void __fastcall CIndex::ReadIndex(LPSTR pCall, int n)
{
	memcpy(pCall, &pIndex[n*16], 16);
	pCall[16] = 0;
}

LPSTR __fastcall CIndex::PointIndex(int n)
{
	return &pIndex[n*16];
}

void __fastcall CIndex::AddBlock(LPCSTR p, int len)
{
	int n = len/16;
...
//---------------------------------------------------------------------------
int __fastcall CIndex::ReadIndex(int handle, FHD *hp)
{
	int		r = TRUE;
	int		BUFSIZE=32768;
...
void __fastcall CIndex::MakeIndex(int handle, FHD *hp)
{
	CWaitCursor w;
...
//---------------------------------------------------------------------------
int __fastcall CIndex::WriteIndex(int handle, FHD *hp)
{
	int		r = TRUE;
	int		BUFSIZE=32768;
...
//---------------------------------------------------------------------------
int __fastcall CLogFind::Add(int n)
{
...
//---------------------------------------------------------------------------
void __fastcall CLogFind::Ins(int n)
{
...
void __fastcall CLogFile::ReadIniFile(LPCSTR pKey, TMemIniFile *pIniFile)
{
	...
	m_LogSet.m_THGMSK = pIniFile->ReadString(pKey, "THGMSK", m_LogSet.m_THGMSK);
...
void __fastcall CLogFile::WriteIniFile(LPCSTR pKey, TMemIniFile *pIniFile)
{
	...
	pIniFile->WriteString(pKey, "THGMSK", m_LogSet.m_THGMSK);
...
void __fastcall CLogFile::MakeIndex(void)
{
	if( !m_Open ) return;
...
void __fastcall CLogFile::MakePathName(LPCSTR pName)
{
	char bf[256];

	m_Name = pName;
	sprintf(bf, "%s%s", sys.m_LogDir, pName);
	SetEXT(bf, ".MDT");
	m_FileName = bf;
}

void __fastcall CLogFile::MakeName(LPCSTR pPathName)
{
	...
	m_Name += ext;
	if( Dir.IsEmpty() ){
		if( GetCurrentDirectory(128, sys.m_LogDir) ){
			if( LastC(sys.m_LogDir) != '\\' ) strcat(sys.m_LogDir, "\\");
            MakePathName(m_Name.c_str());
        }
    }
    else {
		strncpy(sys.m_LogDir, Dir.c_str(), 128);
    }
}

void __fastcall CLogFile::DoBackup(void)
{
...
int __fastcall CLogFile::Close(void)
{
	int r = TRUE;
	if( m_Open ){
		if( m_EditFlag ){
...
void __fastcall CLogFile::InitHeader(void)
{
...
int __fastcall CLogFile::Open(LPCSTR pName, BOOL fNew)
{
	CWaitCursor w;
	Close();
	InitHeader();
	...
	while(1){
		...
		if( !(omode & O_CREAT) && (errno == ENOENT) ){	// １回目でファイルが存在しない場合
            ...
            if( n >= 2 ){
			    if( fNew ){
					if( YesNoMB(sys.m_MsgEng ? "Log file (%s) does not exist, create it ?" : "ログファイル(%s)が見つかりません. 新規に作成しますか ?", pName) != IDYES ) return FALSE;
   				}
...
void __fastcall CLogFile::InitCur(void)
{
	SetLastPos();
}

void __fastcall CLogFile::SetLastPos(void)
{
	m_CurNo = m_fhd.size;
	memset(&m_sd, 0, sizeof(SDMMLOG));
	...
	if( n >= 0 ){
		...
		m_sd.band = B_7;
		m_sd.fq = 28;
		SetMode(&m_sd, g_tLogModeTable[MainVARI->CBMode->ItemIndex]);
//		m_sd.mode = 8;
		m_sd.env = 1;
	}
	CopyAF();
...
int __fastcall CLogFile::GetData(SDMMLOG *sp, int n)
{
...
int __fastcall CLogFile::PutData(SDMMLOG *sp, int n)
{
	if( !m_Open ){
...
LPCSTR __fastcall CLogFile::GetOptStr(int n, SDMMLOG *sp)
{
	switch(n){
		case 0:
			return sp->opt1;
...
void __fastcall CLogFile::SetOptStr(int n, SDMMLOG *sp, LPCSTR pOpt)
{
	switch(n){
		case 0:
...
void __fastcall CLogFile::SetHisRST(SDMMLOG *sp)
{
	int n, tim;
	...
	switch(m_LogSet.m_CopyHis){
		...
		case 2:
			if( m_sd.btime ){
				...
				sprintf(m_sd.ur, "599%02u%02u", tim/60, tim%60);
			}
			break;
	}
}

void __fastcall CLogFile::CopyAF(void)
{
	int n = m_CurNo - 1;
	if( n >= 0 ){
		SDMMLOG sd;
...
void __fastcall CLogFile::SortDate(int bb, int eb)
{
	int		gap, i, j, em;
	SDMMLOG	sd1, sd2;
...
int __fastcall CLogFile::FindSameBand(BOOL fMode)
{
...
int __fastcall CLogFile::FindSameDate(void)
{
	if( !m_sd.call[0] ) return 0;
	char	call[MLCALL+1];
	SDMMLOG	sd;
	...
	for( i = 0; i < m_Find.GetCount(); i++ ){
		...
		if( !strcmp(call, m_sd.call) ){
			...
			int mm = sd.date / 100;
			int dd = sd.date % 100;
			if( (sd.mode == 8) && (sd.band >= B_14) && (yy == utc.wYear) && (mm == utc.wMonth) && (dd == utc.wDay) ) return 1;
		}
	}
	return 0;
}

void __fastcall CLogFile::FindStrSet(CLogFind *fp, LPCSTR pCall)
{
	if( !*pCall ) return;
...
int __fastcall CLogFile::FindCmpSet(CLogFind *fp, LPCSTR pCall)
{
...
int __fastcall CLogFile::FindClipSet(CLogFind *fp, LPCSTR pCall)
{
	if( !*pCall ) return 0;
	if( fp->GetCount() >= FINDMAX ) return 0;
	char	call[MLCALL+1];
...
int __fastcall CLogFile::FindSet(CLogFind *fp, LPCSTR pCall)
{
	fp->SetText(pCall);
	fp->Clear();
...
int __fastcall CLogFile::Find(LPCSTR pCall, int b, int dir)
{
...
int __fastcall CLogFile::IsAlready(LPCSTR pCall)
{
	if( !m_fhd.size ) return -1;
	char	call[MLCALL+1];
...
void __fastcall CLogFile::DeleteAll(void)
{
	m_fhd.size = 0;
	m_CurNo = m_fhd.size;
	m_CurChg = 1;
...
void __fastcall CLogFile::DeleteLast(void)
{
	if( m_fhd.size ){
		m_fhd.size--;
	}
...
void __fastcall CLogFile::Delete(int top, int end)
{
...
void __fastcall CLogFile::Insert(int n, SDMMLOG *sp)
{
...
LPCSTR __fastcall CLogFile::GetDateString(SDMMLOG *sp, int sw)
{
	static char bf[12];
...
LPCSTR __fastcall CLogFile::GetTimeString(WORD d)
{
	static char bf[8];
...
LPCSTR __fastcall CLogFile::GetModeString(BYTE m)
{
	if( m < MODEMAX ){
		return _mode[m];
	}
...
void __fastcall CLogFile::SetMode(SDMMLOG *sp, LPCSTR s)
{
	if( !*s ){
		sp->mode = 0;
		return;
...
LPCSTR __fastcall CLogFile::GetFreqString(BYTE b, short fq)
{
	static char	bf[12];
	int			d;
...
===============================================================
*/
static BYTE __fastcall _bandno_(LPCSTR p)
{
	char	n;
	const char **t;
...
===============================================================
*/
void __fastcall CLogFile::SetFreq(SDMMLOG *sp, LPCSTR p)
{
	BYTE	n;
	char	ub[16], db[16];
	int		ud, d;
...
int __fastcall CLogFile::ReadAscii(SDMMLOG *sp, LPSTR p)
{
	LPSTR	t;
	int	y, m, d;
