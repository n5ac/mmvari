...
//***************************************************************************
// CLogText クラス
__fastcall CLogText::CLogText()
{
	m_Type = 0;
	m_Double = 0;
	m_Delm = 0;
	m_err = 0;
	m_UTC = (sys.m_LCID != 0x0411) ? 1 : 0;
	int i;
...
//---------------------------------------------------------------------------
// テキストファイルのオープン
int __fastcall CLogText::Open(LPCSTR pName)
{
	Close();
	m_Mode = 0;
	m_err = 0;
	...
	if( m_fp != NULL ){
		...
		ErrorMB( sys.m_MsgEng ? "Can't open '%s'":"'%s'がオープンできません.", pName);
		return FALSE;
	}
}
//---------------------------------------------------------------------------
// テキストファイルの作成
int __fastcall CLogText::Create(LPCSTR pName)
{
	Close();
	m_Mode = 1;
	...
	if( m_fp != NULL ){
		...
		ErrorMB( sys.m_MsgEng ? "Can't write to '%s'":"'%s'が作成できません.", pName);
		return FALSE;
	}
}
//---------------------------------------------------------------------------
// テキストファイルのオープン
int __fastcall CLogText::Close(void)
{
	int r = 0;
	if( m_fp != NULL ){
		r = fclose(m_fp);
		m_fp = NULL;
		if( r ){
			ErrorMB( sys.m_MsgEng ? "Can't close '%s'":"'%s'が正しくクローズできませんでした.", m_FileName.c_str());
		}
		else if( m_Mode ){
			InfoMB(sys.m_MsgEng ? "Done (write to '%s')":"'%s'への書きこみを終了しました.", m_FileName.c_str());
		}
	}
	return r ? FALSE : TRUE;
}
//---------------------------------------------------------------------------
// テキストファイルの読み出し
int __fastcall CLogText::Read(SDMMLOG *sp)
{
...
//---------------------------------------------------------------------------
// 出力変換
int __fastcall CLogText::Write(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
//---------------------------------------------------------------------------
// 1行の出力変換
void __fastcall CLogText::MMLOG2Text(LPSTR t, SDMMLOG *sp)
{
	char	bf[512];
	char	dlm = m_Delm ? TAB : ',';
...
//---------------------------------------------------------------------------
int __fastcall GetMonth(LPCSTR p)
{
	for( int i = 1; i <=12; i++ ){
...
//---------------------------------------------------------------------------
// 1行の入力変換
int __fastcall CLogText::Text2MMLOG(SDMMLOG *sp, LPSTR p, int &err)
{
	char	bf[512];
	char	dlm = m_Delm ? TAB : ',';
	LPSTR	t;
	...
	for( i = 0; i < TEXTCONVMAX; i++ ){
		if( (!m_rConv[i].Key.IsEmpty())&&(m_rConv[i].Key != "%EOD") ){
			...
			if( ::Text2MMLOG(sp, t, m_rConv[i].Key ) == FALSE ){
				if( !err ){
					ErrorMB(sys.m_MsgEng ? "An error occurred with conversion type [%s]." : "変換式[%s]でエラーが発生しました.", m_rConv[i].Key.c_str() );
				}
				err++;
			}
		}
...
//---------------------------------------------------------------------------
// LOG200ファイルのオープン
int __fastcall CLog200::Open(LPCSTR pName)
{
	Close();
	m_Index = 0;
	m_err = 0;
...
//---------------------------------------------------------------------------
// LOG200ファイルの作成
int __fastcall CLog200::Create(LPCSTR pName)
{
	Close();
	m_Mode = 1;
	m_err = 0;
...
//---------------------------------------------------------------------------
// LOG200ファイルのクローズ
int __fastcall CLog200::Close(void)
{
	int r = 0;
	if( m_fp != NULL ){
		r = fclose(m_fp);
...
void __fastcall LOG200toMMLOG(SDMMLOG *sp, LPSTR s)
{
	LPSTR	t;
	int y, m, d, h, tim;
	char	bf[256];
...
//---------------------------------------------------------------------------
// LOG200ファイルの読み出し
int __fastcall CLog200::Read(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
void __fastcall MMLOGtoLOG200(LPSTR t, SDMMLOG *sp, int &err)
{
	LPSTR s = t;
	int h, m;
	char	bf[256];
...
//---------------------------------------------------------------------------
// LOG200ファイルの書きこみ
int __fastcall CLog200::Write(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
//---------------------------------------------------------------------------
// HamLogファイルのオープン
int __fastcall CHamLog::Open(LPCSTR pName)
{
	Close();
	m_Index = 0;
	m_err = 0;
...
int __fastcall CHamLog::Close(void)
{
	int r = 0;
	if( m_fp != NULL ){
		if( m_Mode ){		// 書きこみ
...
//---------------------------------------------------------------------------
// HamLogファイルの作成
int __fastcall CHamLog::Create(LPCSTR pName)
{
	Close();
	m_Mode = 1;
	m_err = 0;
...
//----------------------------------------------------------------------
int __fastcall CHamLog::Seek(DWORD Index)
{
...
//----------------------------------------------------------------------
//ＨＡＭＬＯＧのヘッダを作成
BOOL __fastcall CHamLog::MakeHD(void)
{
...
//---------------------------------------------------------------------------
// HamLogファイルの読み出し
int __fastcall CHamLog::Read(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
//---------------------------------------------------------------------------
// HamLogファイルの書きこみ
int __fastcall CHamLog::Write(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
void __fastcall DecBand(LPSTR t, BYTE *pBand)
{
...
void __fastcall HAMLOGtoMMLOG(SDMMLOG *sp, SDHAMLOG *hp, FILE *dbrfp)
{
	...
	else if( !strcmp(bf, Log.m_LogSet.m_THGMSK.c_str()) ){
		strcpy(bf, "GMSK");
	}
...
void __fastcall SetRST(char *pRST, LPCSTR p, BYTE mode)
{
	char	rs[3];
	int		rst;
...
void __fastcall EncBand(BYTE *pBand, LPCSTR pStr)
{
	char	bf[32];
	StrCopy(bf, pStr, 7);
...
//***************************************************************************
// CLogADIF クラス
__fastcall CLogADIF::CLogADIF()
{
	m_bf[sizeof(m_bf)-1] = 0;
}
//---------------------------------------------------------------------------
// ADIFファイルのオープン
int __fastcall CLogADIF::Open(LPCSTR pName)
{
	Close();
	m_Mode = 0;
...
//---------------------------------------------------------------------------
// ADIFファイルの作成
int __fastcall CLogADIF::Create(LPCSTR pName)
{
	Close();
	m_Mode = 1;
...
//---------------------------------------------------------------------------
// テキストファイルのオープン
int __fastcall CLogADIF::Close(void)
{
	int r = 0;
	if( m_fp != NULL ){
		r = fclose(m_fp);
		m_fp = NULL;
		if( r ){
			ErrorMB(sys.m_MsgEng ? "Can't write to '%s'":"'%s'が正しくクローズできませんでした.", m_FileName.c_str());
		}
		else if( m_Mode ){
			InfoMB(sys.m_MsgEng ? "Done (write to '%s')":"'%s'への書きこみを終了しました.", m_FileName.c_str());
		}
...
//---------------------------------------------------------------------------
// ADIFファイルの読み出し
void __fastcall CLogADIF::SetData(SDMMLOG *sp, LPCSTR pKey, LPSTR pData)
{
	int l;
...
//---------------------------------------------------------------------------
void __fastcall CLogADIF::AdjustData(SDMMLOG *sp)
{
	if( !sp->etime ) sp->etime = sp->btime;
	UTCtoJST(sp);
	if( sp->call[0] ){
...
//---------------------------------------------------------------------------
// ADIFファイルの読み出し
int __fastcall CLogADIF::Read(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
int __fastcall CLogADIF::Write(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
//***************************************************************************
// CLogCabrillo クラス
__fastcall CLogCabrillo::CLogCabrillo()
{
	m_bf[sizeof(m_bf)-1] = 0;
}
//---------------------------------------------------------------------------
// LogCabrilloファイルのオープン
int __fastcall CLogCabrillo::Open(LPCSTR pName)
{
	Close();
	m_Mode = 0;
...
//---------------------------------------------------------------------------
// LogCabrilloファイルの作成
int __fastcall CLogCabrillo::Create(LPCSTR pName)
{
	Close();
	m_Mode = 1;
...
//---------------------------------------------------------------------------
// テキストファイルのオープン
int __fastcall CLogCabrillo::Close(void)
{
	int r = 0;
	if( m_fp != NULL ){
...
int __fastcall CLogCabrillo::Read(SDMMLOG *sp)
{
	return FALSE;
}
//---------------------------------------------------------------------------
// 出力変換
//
int __fastcall CLogCabrillo::Write(SDMMLOG *sp)
{
	if( !IsOpen() ) return FALSE;
...
//	"PSK","BPSK","QPSK","HELL","MFSK",
	...
	fprintf( m_fp, "QSO:%6s %2s %04u-%02u-%02u %02u%02u %-13s %-3s %-6s %-13s %-3s %-6s\n",
		...
		tim/60, tim%60, sys.m_CallSign.c_str(),
		sp->ur, SNO,
		sp->call, sp->my, RNO
	);
	return ferror(m_fp) ? FALSE : TRUE;
